...
class Sample:
    ...
    def main(
        ...
        assign_jobs_request = outbound_bot_20191226_models.AssignJobsRequest(**request_params)
        
        runtime = util_models.RuntimeOptions()
        try:
            # 复制代码运行请自行打印 API 的返回值
            response = client.assign_jobs_with_options(assign_jobs_request, runtime)
            print(response)
    ...
    async def main_async(
        ...
        # Add working_time if provided
        if working_time:
            request_params["strategy_json"] = json.dumps({
        ...
        try:
            # 复制代码运行请自行打印 API 的返回值
            await client.assign_jobs_with_options_async(assign_jobs_request, runtime)
        except Exception as error:
            # 此处仅做打印展示，请谨慎对待异常处理，在工程项目中切勿直接忽略异常。
            # 错误 message
            print(error.message)
...
if __name__ == '__main__':
    ...
    json_data = {
        ...
        "contacts": [
            {
                "phoneNumber": "18616698686",
                "name": "时凤卫",
                "referenceId": "sfw0326001"
            }
        ]
        }
    
    job_id = Sample.main(sys.argv[1:],  
                os.getenv('INSTANCE_ID'),
                "7434ebea-63cb-499c-aedb-5ef7a7b19c9e",

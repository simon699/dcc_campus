...
1. **批量查询 call_job_id 列表（处理 call_status 为空或 call_conversation 为空的记录）**
   - 查询条件：`SELECT call_job_id FROM leads_task_list WHERE task_id = X AND call_job_id IS NOT NULL AND call_job_id != '' AND ((call_status IS NULL OR call_status = '') OR (call_conversation IS NULL OR call_conversation = ''))`
   - **重要**：处理 `call_status` 为空或 `call_conversation` 为空的记录，确保能获取到完整的通话数据
   - 这样即使 `call_status` 已经有值（如 Executing），如果 `call_conversation` 为空，也会继续处理
...
4. **批量更新数据库**
   ...
   - 中间状态（如 'Executing'）不保存，继续轮询获取，直到状态变为最终状态
   - 如果查询到记录但更新数为0（说明都是中间状态被跳过），不标记任务完成，继续轮询
   - 如果 `call_job_id` 不为空但 `call_status` 仍为空（可能是中间状态未被保存），不标记任务完成，继续轮询
...
1. **筛选需要创建跟进记录的数据**
   - 查询条件：`SELECT call_job_id, call_conversation, call_status, leads_id FROM leads_task_list WHERE call_job_id IS NOT NULL AND call_status IN ('Succeeded', 'Failed') AND leads_follow_id IS NULL`
   - **重要**：只处理 `call_status` 为最终状态（'Succeeded' 或 'Failed'）的记录
   - 如果是中间状态（如 'Executing'），需要等待状态变为最终状态后再创建跟进记录
...
2. **判断是否需要AI分析**
   ...
   **情况B：call_conversation 为空但 call_status 有值（此时 call_status 必须是 'Succeeded' 或 'Failed'）**
   - 根据 call_status 设置默认值：
     ...
     - `call_status = 'Succeeded'`：**跳过创建跟进记录，等待 call_conversation 获取到后再创建**（因为 call_conversation 可能稍后才会从阿里云获取到）
   
   **情况C：call_status 为空或不是最终状态**
   - 如果 `call_status` 为空：跳过，等待获取到 call_status 后再处理
   - 如果 `call_status` 不是最终状态（如 'Executing'）：跳过，等待状态变为最终状态（'Succeeded' 或 'Failed'）后再处理
...
- call_status 必须是最终状态（'Succeeded' 或 'Failed'）
- call_job_id 必须存在
...
**判断逻辑**
- 查询条件：统计 `task_id = 任务ID` 的所有记录
- 统计需要创建跟进记录的任务：`call_job_id IS NOT NULL AND call_job_id != '' AND call_status IN ('Succeeded', 'Failed')`
- 统计已创建跟进记录的任务：`call_job_id IS NOT NULL AND call_job_id != '' AND call_status IN ('Succeeded', 'Failed') AND leads_follow_id IS NOT NULL`

**更新条件**
- 如果：`需要创建跟进记录的任务数 > 0` 且 `需要创建跟进记录的任务数 == 已创建跟进记录的任务数`
- 则：`UPDATE call_tasks SET task_type = 4 WHERE id = task_id`
- 含义：所有需要创建跟进记录的任务（call_status 为最终状态）都已创建跟进记录，跟进阶段完成

**重要说明**
- 只统计 `call_status` 为最终状态（'Succeeded' 或 'Failed'）的记录
- 中间状态（如 'Executing'）的记录不参与判断，因为它们可能还在进行中，尚未需要创建跟进记录

...
const formatDateValue = (value: string | number | Date | null | undefined): string | null => {
  ...
  return parsed ? parsed.toLocaleString('zh-CN') : null;
};
...
interface FollowupModalProps {
  ...
  selectedTask?: { id: number; task_name: string; task_type: number; create_time: string };
}
...
export default function FollowupModal({ 
  ...
  // 加载“执行记录数据”（分页）
  const fetchRecordsData = useCallback(async (pageToLoad: number = 1) => {
    if (!selectedTaskId) return;
    ...
    try {
      ...
      if (executionResponse.status === 'success') {
        ...
        const totalCount = typeof data.total_jobs === 'number'
          ? data.total_jobs
          : (typeof pagination.total_count === 'number' ? pagination.total_count : 0);
        setTotalJobs(totalCount);
        // 注意：切换到“执行记录”时，仅刷新记录与分页，不刷新顶部概览（不更新 taskInfo/totalJobs）

        const currentPageNum = pagination.page || pageToLoad;
        setCurrentPage(currentPageNum);
        setTotalPages(pagination.total_pages || 1);

        const jobsData = Array.isArray(data.jobs_data) ? data.jobs_data : [];

        const newFollowupRecords: FollowupRecord[] = jobsData.map((job: any, index: number) => {
          const taskList = Array.isArray(job.Tasks) ? job.Tasks : [];
          const task = taskList.length > 0 ? taskList[taskList.length - 1] : undefined;
          ...
          const interestValue =
            job.is_interested !== undefined
              ? job.is_interested
              : job.isInterested !== undefined
              ? job.isInterested
              : null;

          const followTimeSource =
            followData?.follow_time ||
            followData?.new_follow_time ||
            task?.ActualTime ||
            task?.PlanedTime;

          const followupTime =
            formatDateValue(followTimeSource) || '未知时间';

          const nextFollowTime =
            followData?.next_follow_time || followData?.frist_follow_time || null;

          const callDurationSeconds = task?.Duration
            ? Math.floor(task.Duration / 1000)
            : (typeof job.call_duration === 'number' ? job.call_duration : 0);

          return {
            id: job.JobId || job.job_id || `record_${pageToLoad}_${index}`,
            leadName,
            phone,
            isInterested: interestValue,
            remark: followData?.leads_remark || '',
            conversation: task?.Conversation || job.call_conversation || [],
            followupTime,
  ...
  const getTaskStatusText = (taskType: number, _isCompleted: boolean) => {
    switch (taskType) {
      case 1:
        return '已创建';
      case 2:
  ...
  const getTaskStatusColor = (taskType: number, _isCompleted: boolean) => {
            ...
            {(taskInfo || selectedTask) && (
              <div className="mb-6 p-4 bg-white/5 rounded-lg border border-white/10">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-white font-semibold text-lg">{selectedTask?.task_name || taskInfo?.task_name}</h3>
                    <p className="text-gray-400 text-sm">创建时间：{selectedTask ? new Date(selectedTask.create_time).toLocaleString() : taskInfo?.create_time}</p>
                  </div>
                  <div className="flex items-center space-x-3">
                    <span className={`px-3 py-1 rounded text-sm ${getTaskStatusColor(selectedTask?.task_type ?? (taskInfo?.task_type as number), false)}`}>
                      {getTaskStatusText(selectedTask?.task_type ?? (taskInfo?.task_type as number), false)}
                    </span>
                  </div>
                </div>
            ...
            {activeTab === 'records' && (
              <div className="space-y-4">
                ...
                <>
                    ...
                    {/* 分页器 */}
                    {followupRecords.length > 0 && totalPages > 1 && (

...
export default function MonitorDrawer({ isOpen, onClose, callingTasks, onTasksUpdate, onRefreshTaskDetails }: MonitorDrawerProps) {
  ...
  const [jobGroupProgress, setJobGroupProgress] = useState<any>(null); // 任务组进度数据
  ...
  const handleTaskClick = useCallback(async (task: CallingTask) => {
    // 立即打开抽屉，然后异步获取任务组进度数据并更新，避免卡顿
    setSelectedTask(task);
    setJobGroupProgress(null); // 重置进度数据
    setShowCallStatus(true); // 立即打开抽屉，显示加载状态
    
    // 异步获取任务组进度信息，获取到后更新
    try {
      const response = await tasksAPI.describeJobGroup({ task_id: parseInt(task.id) });
      if (response.status === 'success' && response.data) {
        setJobGroupProgress(response.data);
      }
    } catch (error) {
      console.error('获取任务组进度信息失败:', error);
      // 即使失败也继续，CallStatusDrawer 会自己处理
    }
  }, []);
      ...
      {selectedTask && (
        <CallStatusDrawer
          ...
          onClose={() => {
            ...
            setJobGroupProgress(null); // 关闭时清除进度数据
          }}
          ...
          initialJobGroupProgress={jobGroupProgress} // 传入任务组进度数据
        />
      )}
    </>
  );
